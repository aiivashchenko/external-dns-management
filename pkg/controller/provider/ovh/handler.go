...
func NewHandler(c *provider.DNSHandlerConfig) (provider.DNSHandler, error) {
	...
	endpoint, err := c.GetRequiredProperty("OVH_ENDPOINT", "endpoint")
	if err != nil {
		return nil, err
	}
	applicationKey, err := c.GetRequiredProperty("OVH_APPLICATION_KEY", "applicationKey")
	if err != nil {
		return nil, err
	}
	applicationSecret, err := c.GetRequiredProperty("OVH_APPLICATION_SECRET", "applicationSecret")
	if err != nil {
		return nil, err
	}
	consumerKey, err := c.GetRequiredProperty("OVH_CONSUMER_KEY", "consumerKey")
	if err != nil {
		return nil, err
	}
	...
	access, err := NewAccess(&config, c.Metrics, c.RateLimiter)
	if err != nil {
		return nil, err
	}
...
func (h Handler) getZones(cache provider.ZoneCache) (provider.DNSHostedZones, error) {
	rawZones := []client.DNSZone{}
	{
		f := func(zone client.DNSZone) (bool, error) {
	...
	for _, z := range rawZones {
		forwarded := []string{}
		f := func(r client.DNSRecord) (bool, error) {
			if r.Type == dns.RS_NS {
				name := r.Hostname
				if name != z.Name {
